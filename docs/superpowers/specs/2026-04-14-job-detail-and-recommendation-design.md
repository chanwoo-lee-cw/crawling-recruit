...
- `JobService.get_unapplied_job_rows()`: `jobs LEFT JOIN job_details` 쿼리, `LIMIT` 절 없음 (전체 조회)
  반환 필드: `id, company_name, title, location, employment_type, requirements, preferred_points, skill_tags, fetched_at`
  (`job_details` 없는 공고는 detail 필드가 `None`으로 포함)
  "detail 없음" 판정 기준: `fetched_at IS None` (requirements/skill_tags는 nullable이므로 사용 불가)
